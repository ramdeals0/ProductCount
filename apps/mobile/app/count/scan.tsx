...
export default function ScanCountScreen() {
  ...
  const lookupLocalBarcode = useCallback(async (barcode: string) => {
    ...
    return all.filter((p) => {
      ...
      const alternates: string[] = p.barcodeAlternates ? JSON.parse(p.barcodeAlternates) : [];
      return alternates.includes(barcode);
    });
  }, []);

  const saveCount = async (product: typeof localProducts.$inferSelect, qty: number) => {
    if (!sessionId || !activeLocationId || !user) return;

    const variance = calculateVariance(product.expectedQty, qty, product.restrictedType);
    ...
    const existing = await db
      ...
      .where(
        eq(localCountLines.sessionId, sessionId),
      );
